...
class PdoHelper
{
	...
	private function _select($table, $fields = '*', $where = array(), $sort = null, $limit = null)
	{
		...
		$sql = ' FROM ' . $table . $conditions["_where"];
		if (is_array($limit)) {
	...
	public function insert($table, $data)
	{
		...
		$rowCount = $this->exec("INSERT INTO " . $table . " (" . implode(', ', $keys) . ") VALUES (" . implode(', ', $marks) . ")", $values);
		if ($rowCount) {
	...
	public function update($table, $data, $where)
	{
		...
		$conditions = $this->_where($where);
		$rowCount = $this->exec("UPDATE " . $table . " SET " . $update . $conditions["_where"], $conditions["_bindParams"] + $values);
		return $rowCount;
	}
	...
	{
		$conditions = $this->_where($where);
		$rowCount = $this->exec("DELETE FROM " . $table . $conditions["_where"], $conditions["_bindParams"]);
		return $rowCount;
	}
	...
	{
		$conditions = $this->_where($where);
		$count = $this->getColumn("SELECT COUNT(*) FROM " . $table . $conditions["_where"], $conditions["_bindParams"]);
		return $count;
	}
	 ...
	 * @return int|bool
	 */
	public function execNew($_sql, $_array = null)
	{
		$_sql = $this->dealPrefix($_sql);
		if (is_array($_array)) {
			$stmt = $this->db->prepare($_sql);
