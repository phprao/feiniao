...
case 'start': $roomid = $_GET['roomid'];
    ...
    if(get_query_val("fn_feiorder", "term", array("term" => $term, 'roomid' => $roomid)) != ""){
        exit;
    }
...
case "isfly": $term = get_query_val('fn_open', 'next_term', "type = 1 order by term desc limit 1");
    ...
    if(get_query_val("fn_feiorder", "term", array("term" => $term, 'roomid' => $_GET['roomid'])) != ""){
        echo json_encode(array('isfly' => true));
        exit;
    }
...
case "xyftisfly": $term = get_query_val('fn_open', 'next_term', "type = 2 order by term desc limit 1");
    if(get_query_val("fn_flyorder", "money", array("term" => $term, 'roomid' => $_GET['roomid'], 'status' => '未结算')) == ''){
        echo json_encode(array('isfly' => true));
        exit;
    }
    if(get_query_val("fn_feiorder", "term", array("term" => $term, 'roomid' => $_GET['roomid'])) != ""){
...
$cp88 = array('https://678789.com', 'http://660088.com', 'https://c2818.com/main', 'https://789888.com', 'https://vip007.com', 'http://www.888789.com',);
$jufu = array('https://b7700.com', 'https://6785566.com', 'https://www.b8877.com');
