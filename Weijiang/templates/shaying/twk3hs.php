...
//返回注单
 function chazhudan($roomid,$qishu){
	 select_query('fn_twk3order','*',array('roomid'=>$roomid,'term'=>$qishu,'jia'=>'false','status'=>'未结算'));
...
function jisuanjieguo($result,$mingci,$content,$peilv,$jine){
	switch ($mingci){
		case '三军' : 
			return jieguopk($result,$mingci,$content,$peilv,$jine);
			break;
		case '总' : 
			return jieguopk($result,$mingci,$content,$peilv,$jine);
			break;
...
function jieguopk($codes,$mingci,$content,$peilv,$jine){
   $da = '';
   $dan = '';
   $dadan = '';
   $xiaodan = '';
  ...
  if($mingci == '豹子' && $tss == '豹子' && ($codes[0].$codes[1].$codes[2]) == $content){
  $zhi_baozi = true;  
  }else{
  $zhi_baozi = false;
  }  
  if($mingci == '顺子' && $tss == "顺子" && ($codes[0].$codes[1].$codes[2]) == $content){  
  $zhi_shunzi = true;  
  }else{
  $zhi_shunzi = false;
  }     
  if($mingci == '对子' && $tss == "对子" && strpos($codes[0].$codes[1].$codes[2], $content) !== false){
   $zhi_duizi = true; 
  }else{
   $zhi_duizi = false;
  }
  if($mingci == '三杂' && ($tss == "三杂" || $tss == '顺子') && strpos($c, $content) !== false){
   $zhi_sanza = true; 
  }else{
   $zhi_sanza = false;
  }
  if($mingci == '二杂' && strpos($c, $content) !== false){
   $zhi_erza = true;   
  }else{
   $zhi_erza = false; 
  }
	if(($content==$da)||($content==$dan)||($content==$dadan)||($content==$xiaodan)||($content==$dashuang)||($content==$xiaoshuang)||($content==$ts)||($sanjun==true)||($zhi_baozi==true)||($zhi_shunzi==true)||($zhi_duizi==true)||($zhi_sanza==true)||($zhi_erza==true)){
		return $jine*(1-$peilv);			
	}else {
	 	return $jine;
	}	
...
if($mingci=='三军'){
  $ziduan = 'zhi_sanjun';
  $peilv = get_query_val('fn_lottery15',$ziduan,array('roomid'=>$roomid));
  return $peilv;
}elseif($mingci=='总'){
  switch($content){
    case '大': $ziduan = 'da';
...
return $peilv; 
   ...
   switch($content){
    case '豹子': $ziduan = 'tong_baozi';
    break; 
    case '顺子': $ziduan = 'tong_shunzi';
    break;    
    case '对子': $ziduan = 'tong_duizi';
    break;
    case '三杂': $ziduan = 'tong_sanza';
    break;  
    case '二杂': $ziduan = 'tong_erza';
    break; 
  }
$peilv = get_query_val('fn_lottery15',$ziduan,array('roomid'=>$roomid));
return $peilv;
}elseif(is_string($mingci) && is_numeric($content)){
switch($mingci){
    case '豹子': $ziduan = 'zhi_baozi';
    break; 
    case '顺子': $ziduan = 'zhi_shunzi';
    break;    
    case '对子': $ziduan = 'zhi_duizi';
    break;
    case '三杂': $ziduan = 'zhi_sanza';
    break;  
    case '二杂': $ziduan = 'zhi_erza';
    break; 
  }
$peilv = get_query_val('fn_lottery15',$ziduan,array('roomid'=>$roomid));
return $peilv;
