...
if ($_POST) {
    ...
    //验证注册是否为空账号密码
    if ($name == '' || $pwd == '' || $headimg == '') {
        echo "<script>alert('用户名和密码/头像不能为空！请重新输入！');location.href='web_reg.php';</script>";
        exit;
    }
    //验证账号是否重复
    $name1 = $_POST['name'];
    $num_rows = $dbpdo->count('fn_user', "username = '$name1'");
    if ($num_rows >= 1) {
        echo "<script>alert('该用户名已被注册！');location.href='web_reg.php';</script>";
        exit;
    } else {
        ...
        $exec = $dbpdo->query($sql);
        if ($exec) {
            echo "<script>alert('注册成功！');location.href='web_login.php'</script>";
        } else {
            echo "<script>alert('注册失败,请重试。');location.href='web_reg.php'</script>";
        }
    }
}
?>
...
<body>
    <div class="back-main">
        <form action='web_reg.php' method='post'>
            <ul class="home_url">
                <div align="center" style="margin-top:-50%;"><img src="/upload/bdkjlogo.png" style="width:50%" /></div>
                <li>
                    <div class="inputtxt"><input name="name" type="text" placeholder="用户名" title="用户名"></div>
                </li>
                <li>
                    <div class="inputtxt"><input type="password" name="pwd" placeholder="密码" title="密码"></div>
                </li>
                <li>
                    <!--//设置头像-->
                    <div class="rds">
                        <!--<center><span>选择头像</span></center>-->
                        <label id="headimg0"><input checked=checked name='headimg' type="radio" value="/Style/images/0.png"></label>
